...
- **What it does:** This component takes a data set and uses its information to draw three maps, and place markers on them. The maps show the city of Seattle and the markers represent the location of new building construction. The the size of the marker corresponds to the number of collisions that happened in the period before, during or after the construction period. 

- **Inputs:** 
  - data (pandas dataframe) This module takes as input a Pandas dataframe with the following fields:
...
- **How it interacts with other components:** This module interacts with other modules as follows:
  + *interactions_functionality*: The draw\_markerss module is executed within the interactions\_functionality module, which also generates the data table input of this module.. 
...
- **Outputs:** 
  ...
  - **How it interacts with other components:** This component uses database created in by the process_data and table_builder modules. Recieves query created in the "build\_query" component and uses it to query database. It builds maps using the draw_markers component and visualizes maps in the UI component 
...
- **Inputs:**  User inputs: Each use case allows users to select an option from a drop down menu in the user interface. These include:  building_category (str), building_year (int), collision_interval (int) collision_severity (str), collision_type (str), radius_from_building (int) and map_detail(str): Allows user to indicate if they prefer a map style with low detail or high detail.
